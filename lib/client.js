window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function restoreCommanders(ids) {
			for (const sessionId of ids) {
				try {
					...
					update((s) => {
						s.commanders.set(sessionId, { sessionId, cursor: probe.cursor, roster, outstanding: 0, dispatchedTotal: 0, lastBatchAt: 0, error: "", commanderHops: 0 });
		...
		function buildReportText(commanderId) {			const tasks = [...state.tasks.values()].filter((task) => task.commanderId === commanderId).reverse();
