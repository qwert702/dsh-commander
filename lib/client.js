...
// ============================================================================
			...
			for (const block of Array.isArray(blocks) ? blocks : []) {
				...
				if (raw.indexOf(",") !== -1) {
					for (const part of raw.split(",")) {
						...
						if (target !== "") out.push({ ...block, target, src });
					}
					src += 1;
					continue;
				}
		...
		function normTaskKey(text) {
			// Cap the key length to match openTask's fullText budget: admission
			// compares FULL item.task while history stores TRUNCATED fullText —
			// slicing both sides keeps long-task dedup aligned instead of blind.
			return String(text ?? "").toLowerCase().replace(/\s+/g, " ").trim().slice(0, 2000);
		}
		...
		function persistTasks() {
			try {
				...
				if (all.length > TASK_HISTORY_LIMIT) {
					...
					const live = all.filter((task) => !isTerminalStatus(task.status));
					...
					keep = all.filter((task) => !droppedTerminals.has(task.id));
					if (keep.length > TASK_HISTORY_LIMIT * 2) keep = keep.slice(keep.length - TASK_HISTORY_LIMIT * 2); // hard backstop
			...
			for (const task of [...state.tasks.values()]) {
				...
				if (task.cancelRequested === true) {
					...
					note = (task.continuations | 0) > 0
						...
						: "完成（输出被 token 上限截断）";
				} else if (RESUMABLE_END_REASONS.includes(reason)) {
					// Only whitelist reasons count as abnormal ends. The real
					// harness finishes turns with reason "completed" — the old
					// open-ended `reason !== "stop"` check marked EVERY normal
					// completion failed and re-ran it via failover.
					status = "failed";
					note = (task.continuations | 0) > 0
