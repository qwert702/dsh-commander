...
// ============================================================================
		 ...
		 */
		const ENGINE_BUILD = "dsh-commander client r28";
		...
		/**
		 * Remove fenced code blocks (```…```) from a text. Mail/roundtable
		 * parsing only — a fenced <dsh-mail> is a DOCUMENTED example (worker
		 * echoing the hint) far more often than a real send, while dispatch
		 * blocks are still parsed from the raw text because models sometimes
		 * fence their real dispatches.
		 */
		...
		function parseRoundtableBlocks(text) {
			...
			if (typeof text !== "string" || source.indexOf("<dsh-roundtable") === -1) return blocks;
			RT_RE.lastIndex = 0;
			let match;
			while ((match = RT_RE.exec(source)) !== null) {
				const attrs = {};
				ATTR_RE.lastIndex = 0;
				let attr;
		 ...
		 * Fenced blocks are stripped and the briefing's own example body is
		 * ignored — a commander reciting the briefing verbatim used to fire a
		 * real bogus mail at #2/#3.
		 */
		function parseMailBlocks(text) {
			const mails = [];
			const source = stripFencedCode(text);
			if (typeof text !== "string" || source.indexOf("<dsh-mail") === -1) return mails;
			MAIL_RE.lastIndex = 0;
			let match;
			while ((match = MAIL_RE.exec(source)) !== null) {
				const attrs = {};
				ATTR_RE.lastIndex = 0;
				let attr;
				while ((attr = ATTR_RE.exec(match[1])) !== null) attrs[attr[1].toLowerCase()] = attrValue(attr);
				const body = (match[2] ?? "").trim();
				if (body === "" || body === "消息内容") continue;
				mails.push({
		...
		/**
		 * The dedup key BOTH sides must agree on. Admission sees the raw block
		 * text; history stores `truncateText(task, maxTaskChars)`. Comparing
		 * normTaskKey(raw) against normTaskKey(stored) diverges for texts longer
		 * than the budget (raw whitespace collapses differently and the stored
		 * copy carries a trailing "…"), so the admission side first truncates
		 * to the exact string that WILL be stored, then normalizes that.
		 */
		function dedupKeyOf(text) {
			const budget = Math.max(50, positiveInt(state.config.maxTaskChars, 4000));
			return normTaskKey(truncateText(text, budget));
		}
		...
		function evaluateBatch(blocks, st, config, activeTaskKeys) {
			...
			if (config.dedupDispatch !== false && (items.length > 1 || (activeTaskKeys !== undefined && activeTaskKeys.size > 0))) {
				...
				const kept = [];
				for (const block of items) {
					const key = dedupKeyOf(block.task);
		...
		function briefingText(roster) {
			...
			lines.push(
				...
				...(state.config.mailHintOnDispatch === false ? [] : [
					"worker 间协作消息（可选）——需要把中间结论同步给另一个 worker（而不经指挥官转达）时，在回复中原样输出：",
					'<dsh-mail to="#2,#3" subject="接口约定" leases="src/api.ts">消息内容</dsh-mail>',
					'- to 填花名册别名或会话 id，"all" 发给全部；leases 是可选的文件占用声明（逗号分隔的路径，让其他 worker 避免同时改这些文件）；消息会由插件送达并唤醒收件会话。',
				]),
			);
			return lines.join("\n");
		}

		/** One-line mail hint appended to an OUTGOING task prompt (never stored in the task record). */
		function workerMailHint() {
			if (state.config.mailHintOnDispatch !== true) return "";
			return "\n\n[协作提示] 需要把中间结论同步给其他 worker（或指挥官）时，可在回复中原样输出 <dsh-mail to=\"#别名或id\" subject=\"主题\">消息</dsh-mail>，插件会代为送达。没有需要同步的内容就正常输出结果。";
		}
		//#endregion
		...
		const STORAGE_AWAITING = "dsh-commander.awaiting";
		...
		/**
		 * Confirmation-mode parked batches persist alongside the task table:
		 * without this a reload evaporated every awaiting batch (the panel just
		 * showed nothing, the model believed its work was dispatched).
		 */
		function persistAwaiting() {
			try {
				const entries = [];
				for (const record of state.commanders.values()) {
					if (record.awaitingConfirm !== undefined && record.awaitingConfirm !== null) {
						entries.push({ sessionId: record.sessionId, awaiting: record.awaitingConfirm });
					}
				}
				localStorage.setItem(STORAGE_AWAITING, JSON.stringify(entries));
			} catch {}
		}

		function readPersistedAwaiting() {
			try {
				const raw = JSON.parse(localStorage.getItem(STORAGE_AWAITING) ?? "[]");
				const map = new Map();
				if (!Array.isArray(raw)) return map;
				for (const entry of raw) {
					if (entry === null || typeof entry !== "object" || typeof entry.sessionId !== "string" || entry.sessionId === "") continue;
					const awaiting = entry.awaiting;
					if (awaiting === null || typeof awaiting !== "object" || !Array.isArray(awaiting.items)) continue;
					const items = awaiting.items.filter((item) => item !== null && typeof item === "object" && typeof item.task === "string" && item.task !== "");
					if (items.length === 0) continue;
					map.set(entry.sessionId, { items, batchId: typeof awaiting.batchId === "string" ? awaiting.batchId : null, at: Number(awaiting.at) || Date.now() });
				}
				return map;
			} catch {
				return new Map();
		...
		function restoreTasks() {
			...
			restoredAwaiting = readPersistedAwaiting();
		}

		/** Parked confirmation-mode batches read back during restoreTasks, re-attached when their commander returns. */
		let restoredAwaiting = new Map();

		/** Attach any persisted parked batch to a freshly (re)built commander record. */
		function attachRestoredAwaiting(sessionId, record) {
			const awaiting = restoredAwaiting.get(sessionId);
			if (awaiting === undefined) return;
			restoredAwaiting.delete(sessionId);
			record.awaitingConfirm = awaiting;
			console.info(LOG_PREFIX, "恢复挂起的确认批次（" + awaiting.items.length + " 项）：", sessionId);
		}
		...
		let polling = false;
		/**
		 * Bumped on shutdown so an IN-FLIGHT poll of the old engine copy aborts
		 * at the next stage boundary instead of finishing its pass against a
		 * frozen state (the seconds-wide HMR takeover window).
		 */
		let pollEpoch = 0;
		...
		async function restoreCommanders(ids) {
			...
			for (const sessionId of ids) {
				try {
					...
					attachRestoredAwaiting(sessionId, state.commanders.get(sessionId));
				} catch (error) {
		...
		async function drainPendingRestores() {
			...
			for (const [sessionId, attempts] of [...pendingRestore]) {
				if (state.commanders.has(sessionId)) {
					// The user manually activated this id while it was pending:
					// the live record wins — never replace it with a re-restore.
		...
		async function activate(sessionId) {
			...
			// Manual activation supersedes any queued self-heal for this id —
			// letting drainPendingRestores run later would REPLACE this fresh
			// record (cursor re-anchored, counters reset, batches re-seeded).
			pendingRestore.delete(sessionId);
			attachRestoredAwaiting(sessionId, state.commanders.get(sessionId));
			ensureTimer();
		...
		/**
		 * DISPLAY count for the badge / global pill: every task that has not
		 * settled yet — 排队/定时/等依赖 included, otherwise a commander with 5
		 * queued tasks showed an empty (lying) badge. Admission gating keeps
		 * using countOutstanding.
		 */
		function countPending(commanderId) {
			let count = 0;
			for (const task of state.tasks.values()) {
				if (task.commanderId === commanderId && !isTerminalStatus(task.status)) count += 1;
			}
			return count;
		}
		...
		async function poll() {
			...
			const aborted = () => epoch !== pollEpoch || state.booted !== true;
			polling = true;
			try {
				...
				if (!aborted()) await fireScheduledTasks();
				if (!aborted()) await monitorTasks();
				if (!aborted()) await pollTakenOverWatch();
				if (!aborted()) await drainPendingRestores();
				if (!aborted()) await drainWaitingQueues();
			} finally {
				polling = false;
			}
			if (aborted()) return;
		...
		async function processCommander(commanderId) {
			...
			for (const t of state.tasks.values()) {
				...
				const key = dedupKeyOf(t.fullText ?? t.excerpt);
		...
		async function performSend(record, taskId) {
			...
			// The mail hint rides on the OUTGOING prompt only: the stored record
			// (and its dedup key) stays the pure task text.
		...
		async function releaseAwaiting(sessionId, release) {
			...
			// The outstanding budget may have filled while the batch sat parked.
			// Overflow items are RE-PARKED (with the same batch id) instead of
			// being dropped, so nothing silently vanishes.
			const room = Math.max(0, positiveInt(state.config.maxOutstanding, 5) - countOutstanding(sessionId));
			const admitted = [];
			const overflow = [];
			for (const item of pending.items) {
				(admitted.length < room ? admitted : overflow).push(item);
			}
			for (const item of admitted) {
				const key = dedupKeyOf(item.task ?? item.excerpt);
			...
			if (overflow.length > 0) {
				update((s) => {
					const rec = s.commanders.get(sessionId);
					if (rec === undefined) return;
					rec.awaitingConfirm = { items: overflow, batchId: pending.batchId, at: Date.now() };
				});
				console.warn(LOG_PREFIX, "放行时并发已满，" + overflow.length + " 个任务继续挂起等待下次放行");
				notifyUser("指挥官 · 任务部分放行", String(overflow.length) + " 个任务因并发上限继续等待");
			}
		}
			...
			return react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, {
				children: [
					react_jsx_runtime.jsxs("button", {
						...
						children: [
							"指挥官",
							countPending(sessionId) > 0 ? react_jsx_runtime.jsx("span", { className: "dsh-cmdr-count", title: "进行中 " + String(runningCount) + " · 未结算 " + String(countPending(sessionId)), children: String(countPending(sessionId)) }, "count") : null,
						],
					}, "badge"),
			...
			return react_jsx_runtime.jsxs("div", {
				...
				children: [
					...
					state.active.map((sessionId) => {
						const count = countPending(sessionId);
		...
		exports.parseMailBlocks = parseMailBlocks;
		exports.parseRoundtableBlocks = parseRoundtableBlocks;
		exports.stripFencedCode = stripFencedCode;
		exports.truncateText = truncateText;
		exports.expandBlocks = expandBlocks;
		exports.evaluateBatch = evaluateBatch;
		exports.normTaskKey = normTaskKey;
		exports.dedupKeyOf = dedupKeyOf;
		exports.buildRoster = buildRoster;
		exports.briefingText = briefingText;
		exports.workerMailHint = workerMailHint;
		exports.state = state;
		exports.boot = boot;
