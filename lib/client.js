	...
	factory: (require) => {
		...
		async function processCommander(commanderId) {
			...
			for (const item of verdict.items) {
				await dispatchTask(record, item, batchId).catch((error) => console.warn(LOG_PREFIX, "派发异常：", errorMessage(error)));
			}
		}
		 ...
		 * auto-create (inheriting the commander's cwd; the title comes from the
		 * block, or an auto `[T#n] 摘要` label so the sidebar shows what each
		 * fresh window is working on).
		 */
		async function dispatchTask(record, item, batchId) {
		...
		async function monitorTasks() {
			...
			for (const task of [...state.tasks.values()]) {
				...
				if (task.cancelRequested === true) {
					status = "failed";
					note = "已手动取消";
				} else if (reason === "max-tokens") note = "完成（输出被 token 上限截断）";
				else if (reason !== "stop") {
					status = "failed";
		...
		const STATUS_LABEL = { sending: "发送中", running: "运行中", done: "已完成", failed: "失败", blocked: "已阻塞", "taken-over": "已接管" };
