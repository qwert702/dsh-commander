...
// ============================================================================
		...
		function evaluateBatch(blocks, st, config) {
			...
			if (newWorkerCap < blocks.filter((block) => (block.target ?? "") === "").length) {
				const limited = [];
				let seenNew = 0;
			...
			return {
				action: admitted.length > 0 || autoDropped > 0 ? "execute" : "empty",
				items: admitted,
				dropped: Math.max(0, blocks.length - admitted.length - autoDropped),
				autoDropped,
			};
		}
		//#endregion
		...
		function pickFailoverCandidate(record, excludeWorkerId) {
			...
			const rows = [...(record.roster ?? [])]
				.filter((row) => row.id !== excludeWorkerId && row.id !== record.sessionId && !state.active.includes(row.id))
		...
		async function restoreFromSources() {
			...
			await restoreCommanders(ids.filter((id) => snapshot.byId[id] !== undefined));
			...
			for (const sessionId of ids) {
				try {
					...
					if (!probe.ok) {
						...
						// the entire history as fresh dispatches.
						console.warn(LOG_PREFIX, "游标探测失败，跳过恢复该指挥官以防历史重放：", sessionId);
						continue;
					}
					update((s) => {
						s.commanders.set(sessionId, { sessionId, cursor: probe.cursor, roster, outstanding: 0, dispatchedTotal: 0, lastBatchAt: 0, error: "", commanderHops: 0, reportedBatches: seedReportedBatches(sessionId) });
		...
		async function processCommander(commanderId) {
			...
			// Mail blocks first: they are fire-and-forget and don't consume the
			// batch budget.
			const mailBlocks = (data.events ?? []).flatMap((event) => parseMailBlocks(event.text));
			if (mailBlocks.length > 0) await deliverMailFromBlocks(record, commanderId, mailBlocks);
			// Roundtable: fire-and-forget, the host runs the two-round debate.
			...
			for (const rt of rtBlocks) {
				...
				fetch("/api/dsh-commander/roundtable", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ topic: rt.topic, body: rt.body, count: rt.count }) }).then((result) => {
					if (result.ok !== true || typeof result.minutes !== "string") {
						console.warn(LOG_PREFIX, "圆桌讨论失败：", result.error?.message ?? result.message ?? 'unknown');
						update((s) => {
							const r = s.commanders.get(commanderId);
							if (r !== undefined) r.roundtableActive = undefined;
						});
						return;
					}
					return face.prompt([{ type: "text", text: result.minutes }], "queue").then(() => {
			...
			if ((verdict.autoDropped | 0) > 0) {
				console.warn(
					LOG_PREFIX,
					"一次性新建会话数已达上限（" + String(zeroAllowedInt(state.config.maxNewWorkersPerBatch, 3, 0, 16)) + "），丢弃 " + String(verdict.autoDropped) + " 个自动新建任务块",
				);
			}
			...
			if (state.config.confirmDispatch === true && verdict.items.length > 0) {
				update((s) => {
					...
					rec.awaitingConfirm = { items: verdict.items, batchId, at: Date.now() };
				});
				...
				if (preview !== undefined) {
					...
					if (currentLoad >= wCap) {
						const candidates = (record.roster ?? [])
							...
							.sort((a, b) => (loads.get(a.id) | 0) - (loads.get(b.id) | 0));
						const free = candidates.find((r) => (loads.get(r.id) | 0) < wCap && runtime.sessions.binding(r.id)?.session !== undefined);
						if (free !== undefined) {
