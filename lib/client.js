window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * @returns the parsed blocks [{ target, title, task }] — task is trimmed and non-empty.
		 */
		...
		const state = {
			...
			commanders: new Map(),
			/** Every known task keyed by task id (kept for the panel history). */
			tasks: new Map(),
			taskSeq: 0,
			panelOpenFor: null,
		};
		...
		/** Mutate the engine state through `fn`, then notify subscribers. */
		function update(fn) {
			fn(state);
			version += 1;
		...
		async function restoreCommanders(ids) {
			for (const sessionId of ids) {
				try {
					...
					update((s) => {
						s.commanders.set(sessionId, { sessionId, cursor, roster, outstanding: 0, dispatchedTotal: 0, lastBatchAt: 0, error: "" });
		...
		function hasOutstanding() {
			for (const task of state.tasks.values()) {
				if (task.status === "sending" || task.status === "running") return true;
			}
			return false;
		}
		...
		async function processCommander(commanderId) {
			...
			const blocks = [];
			for (const event of data.events ?? []) blocks.push(...parseDispatchBlocks(event.text));
			if (blocks.length === 0) {
			...
			for (const item of verdict.items) {
				await dispatchTask(record, item).catch((error) => console.warn(LOG_PREFIX, "派发异常：", errorMessage(error)));
			}
		}

		/** Create the task record up-front so the panel shows the attempt even when resolution fails. */
		function openTask(record, item) {
		...
		function markTask(taskId, status, detail) {
			update((s) => {
				...
				if (task === undefined || task.status === "done" || task.status === "failed" || task.status === "blocked") return;
				task.status = status;
		...
		/**
		 * Resolve the target for one block and deliver the task. Resolution
		 * order: roster alias -> full session id -> exact display-title match ->
		 * auto-create (inheriting the commander's cwd, renamed when titled).
		 */
		async function dispatchTask(record, item) {
			const taskId = openTask(record, item);
			const sessions = runtime.sessions;
			const list = sessions.list.getSnapshot();
			...
			const face = sessions.binding(workerId)?.session;
			if (face === undefined) {
				markTask(taskId, "failed", "目标会话不可达（不在列表中）：" + truncateText(workerId, 40));
				return;
			}

			// Baseline BEFORE prompting: the receipt only summarizes what the
			// worker produced AFTER our task landed.
			let baseline = 0;
			try {
				const probe = await fetchEvents(workerId, 0, 1);
				if (probe.ok === true) baseline = Number(probe.lastAssistantSeq ?? probe.lastSeq ?? 0);
			} catch {}

			const taskText = truncateText(item.task, positiveInt(state.config.maxTaskChars, 4000));
			let accepted = false;
			let failure = "";
			try {
			...
			console.info(LOG_PREFIX, "任务已派发 →", workerId, "：", truncateText(item.task, 60));
		}
		 ...
		 * commander (which wakes it to continue orchestrating).
		 */
		async function monitorTasks() {
			...
			for (const task of [...state.tasks.values()]) {
				...
				const parts = [];
				for (const event of data.events ?? []) parts.push(event.text);
				let summary = parts.join("\n\n").trim();
				if (summary === "") summary = "（该回合没有产生文本输出）";
				summary = truncateText(summary, positiveInt(state.config.summaryMaxChars, 800));
				let status = "done";
				let note = "完成";
				if (reason === "max-tokens") note = "完成（输出被 token 上限截断）";
				else if (reason !== "stop") {
					status = "failed";
					note = "回合异常结束（reason=" + reason + "）";
				}
				finishTask(task, status, note + "\n摘要：" + summary);
		...
		function finishTask(task, status, detail) {
			if (task.status === "done" || task.status === "failed" || task.status === "blocked") return;
		...
		const STATUS_LABEL = { sending: "发送中", running: "运行中", done: "已完成", failed: "失败", blocked: "已阻塞" };
		...
		/** The fixed dropdown panel: tasks, roster, and lifecycle actions. */
			...
			return react_jsx_runtime.jsxs("div", {
				...
				children: [
					...
					react_jsx_runtime.jsx("div", {
						...
						children: "派发格式：<dsh-dispatch target=\"#1\" title=\"标题\">任务</dsh-dispatch> · 结果将以「[指挥官回执]」注入本会话。",
					}, "foot"),
				],
			});
		});
		 ...
		 * `conversation.session.header.actions` list seat (the same seat the
		 * compressor and continue-on-limit ride) rendering the commander badge /
		 * panel. The engine lives at module scope, so orchestration continues
