window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * Resolve the target for one block and hand the resolved task to the
		 * send pipeline. Resolution order: roster alias -> full session id ->
		 * exact display-title match -> auto-create (inheriting the commander's
		 * cwd; the title comes from the block, or an auto `[T#n] 摘要` label so
		 * the sidebar shows what each fresh window is working on).
		 */
		...
		const STATUS_LABEL = { sending: "发送中", running: "运行中", waiting: "排队中", done: "已完成", failed: "失败", blocked: "已阻塞", "taken-over": "已接管" };
