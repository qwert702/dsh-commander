 ...
 *
 *   GET  /api/dsh-commander/config   { config }              鈥?resolved settings
 *   POST /api/dsh-commander/inject   { sessionId, text }     鈥?silent briefing injection
 *   GET  /api/dsh-commander/events   ?sessionId&cursor&limit 鈥?settled assistant tail
 *
 * The orchestration 鈥?roster building, protocol-block parsing, dispatching,
 * worker watching, receipt delivery 鈥?happens entirely in the browser half
 ...
 * in the UI, and report the HIGHEST assistant seq in the whole log regardless
 * of cursor/limit 鈥?the activation-time anchor a fresh commander advances its
 * cursor to, so pre-activation outputs can never be re-executed.
 * @param session - the live host session.
 * @param cursor - exclusive seq lower bound.
 * @param limit - maximum number of events returned.
 * @returns { events, lastEnd, lastAssistantSeq } 鈥?projected entries, the
 *   final turn/end fact, and the log-wide tail anchor.
 */
function projectAssistantTail(session, cursor, limit) {
  ...
  return { events, lastEnd, lastAssistantSeq }
}
 ...
 * the text as ONE plugin-sourced user message (no turn is opened 鈥?the model
...
async function handleEvents(req, res, ctx, source) {
  ...
  const { events, lastEnd, lastAssistantSeq } = projectAssistantTail(session, cursor, limit)
  send(res, 200, { ok: true, sessionId, events, lastSeq: session.seq, lastAssistantSeq, lastEnd })
}
